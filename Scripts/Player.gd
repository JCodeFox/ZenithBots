...
@onready var game_over_popup :Object= get_node("../GameOver")
...
func _physics_process(delta):
	if health<=0:
		...
		if not game_over_popup.visible:
...
func update_health_bar():
	get_tree().get_root().get_node("World/HealthBar").value=health

func update_clock():
	var time_percent=$Timer.wait_time-$Timer.time_left/$Timer.wait_time
	get_tree().get_root().get_node("World/Clock/Hand").rotation.y=(-2*PI)*time_percent
